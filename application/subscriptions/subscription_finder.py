...
class SubscriptionFinder(object):
    ...
    def get_subscription_id_or_free_tier_id(cls, \
        ...
        logging.info(f"Finder.get_subscription_id_or_free_tier_id: creator_id: {creator_id}, subscriptionid: {subscriptionid}")
        if subscriptionid is None:
            if creator_id is None:
                loaded = Loader.load_by_name(SubscriptionEntity, "Free tier")
                logging.info(f"Finder.get_subscription_id_or_free_tier_id: loaded.thing: {loaded.thing}")
                ...
                with engine.connect() as c:
                    ...
                    for row in rs:
                        sid = row[0]
                engine.dispose()
                logging.info(f"Finder.get_subscription_id_or_free_tier_id: sid: {sid}")
                subscriptionid = sid
        logging.info(f"Finder.get_subscription_id_or_free_tier_id: subscriptionid: {subscriptionid}")
